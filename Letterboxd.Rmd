...
dropboxToken <- Sys.getenv('DROPBOX_TOKEN')
print(str_c('Checking system variables: ', dropboxToken))

## Download letterboxd diary from dropbox
letReq <-  request("https://content.dropboxapi.com/2/files/download") %>% 
    req_auth_bearer_token(dropboxToken) %>% 
    req_method('POST') %>%
    req_headers(
        'Dropbox-API-Arg' = jsonlite::toJSON(
            list(path="/R/trakt/letterboxd/diary.csv"),
            auto_unbox = TRUE)
        ) %>% 
    req_perform()

writeBin(resp_body_raw(letReq), 'tempLet.csv')
diary <- read_csv('tempLet.csv') %>% 
    select(title = Name, date = `Watched Date`, rating = Rating, tags = Tags) %>%
    arrange(desc(date))
unlink('tempLet.csv')

## Then do the same for goodreads
goodReq <-  request("https://content.dropboxapi.com/2/files/download") %>% 
    req_auth_bearer_token(dropboxToken) %>% 
    req_method('POST') %>%
    req_headers(
        'Dropbox-API-Arg' = '{"path":"/R/goodreads/goodreads.csv"}'
            ) %>% 
    req_perform()
        
writeBin(resp_body_raw(goodReq), 'tempGood.csv')
goodreads <- read_csv('tempGood.csv') %>%
